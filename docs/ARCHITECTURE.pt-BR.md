...
## 6. Escopo da Release 3

A terceira release adiciona o **CCF (Cross-Correlation Function)** com varredura de lags à Camada
de Análise, como método plugável adicional via `CorrelationMethod` (`analysis.methods: [..., ccf]`).
Para cada par $(d_i, d_j)$, o coeficiente de $\mathbf{V}$ passa a ser o de maior módulo entre os
retornos log deslocados de $-\text{max\_lag}$ a $+\text{max\_lag}$ minutos
(`analysis.ccf_max_lag`, padrão 5), preservando a comparabilidade célula a célula já estabelecida
na Release 2; o lag correspondente é persistido na tabela de pares (`lag`, em minutos) sem alterar
