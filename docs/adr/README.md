# Architecture Decision Records

Decisões arquiteturais relevantes do projeto, no formato [MADR](https://adr.github.io/madr/) curto.

| ADR | Título | Status |
|---|---|---|
| [0001](0001-pipeline-em-camadas.md) | Pipeline em camadas (pipes-and-filters) | Aceito |
| [0002](0002-contrato-da-fonte-de-dados.md) | Contrato da fonte de dados e cliente REST do qData_service | Aceito |
| [0003](0003-armazenamento-parquet-por-mutabilidade.md) | Armazenamento em Parquet separado por mutabilidade | Aceito |

## Template

```markdown
# NNNN — Título

- Status: Proposto | Aceito | Substituído por ADR-XXXX
- Data: YYYY-MM-DD

## Contexto

## Decisão

## Consequências

## Alternativas consideradas
```
